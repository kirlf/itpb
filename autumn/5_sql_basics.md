...
3. **Базовое отношение** - это именованное отношения, которое не является производным других отношений (не зависит от существования других отношений).

4. **Производное отношение** - это отношение, которое определяется через другие именованные отношения.

5. **Атрибут** - это поименованный столбец отношения.

6. **Кортеж** - это строка отношения (запись).

7. **Первичный ключ** (primary key) - это уникальный идентификатор отношения (таблицы).

8. **Внешний ключ** (foreign key) - это ключ, объявленный в базовом отношении, который при этом ссылается на первичный ключ того же самого или другого базового отношения.

Итак, вокабуляр подготовлен.

## Подготовка БД для практических примеров

В рамках семинара мы бдем использовать относительно легковесную СУБД [SQLite3](https://sqlite.org/index.html).
...
Кроме операций создания (`INSERT`) и чтения (`SELECT`) записей, современные СУРБД также поддерживают оперции обновления (`UPDATE`) и удаления (`DELETE`) записей. Задание: провести операции обновления и удаления на представленных выше таблицах.
