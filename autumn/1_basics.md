...
Язык C - это язык со **статической типизацией**, а значит тип данных указывается в явном виде перед названием пременной. В нашем случае, это `int` - целочисленное значение. Тип данных говорит компьютеру сколько **байт** он должен выделить в своей памяти для хранения той или иной переменной. Статическая типизация ускоряет работу программ. Подробнее см. в [Керниган, Ричи]:
- "2.4 Объявления".

> Кстати, очень красочно про переменные (на примере языка Java) расказывается в данном учебном материале: ["Элли, переменные и их типы" (javarush.ru)](https://javarush.ru/quests/lectures/questsyntax.level00.lecture04). Советую прочитать.
...
Более подробно об этом можно прочесть в [Керниган, Ричи] в главах:
...
В ходе написания кода довольно удобно выносить некоторые участки кода в, так сказать, "черные ящики", которыми после можно пользоваться, не задумываясь 
об их наполнении. Роль таких "черных ящиков" в С играют **функции**.
...
Более подробно см. [Керниган, Ричи]:
...
А, во-вторых, мы собственно, и воспользовались одним из инструментов вывода - функцией [```prrintf```](https://linux.die.net/man/3/printf).

См. подробнее следующие части в [Керниган, Ричи]:
...
Например, у нас есть дни недели:
...
Более подробно см. в [Керниган, Ричи]:
- "2.6 Операции отношения и логические операции";
...
*Рис. 5. Иллюстрация процесса компиляции. Гриффитс, Гриффитс: Изучаем программирование на C, - М. Эксмо, 2013. - С. 38.*
